...
from GlobalAttention import SpatialAttention as SPATIAL_NET
...
class NEXT_STAGE_G(nn.Module):
    ...
    def define_module(self):
        ngf = self.gf_dim
        self.spatial_att = SPATIAL_NET(ngf, self.ef_dim)
    ...
    def forward(self, h_code, c_code, word_embs, mask, cap_lens):
        ...
        """
        self.spatial_att.applyMask(mask)
        c_code, att = self.spatial_att(h_code, word_embs)
        ...
        h_code_new = torch.cat((h_code_new, h_code), 1)
        
        
        #another way to combine dm and control
        #self.spatial_att.applyMask(mask)
        #c_code, att = self.spatial_att(h_code, word_embs)
        #c_code_channel, att_channel = self.channel_att(c_code, word_embs, h_code.size(2), h_code.size(3))
               
        
        h_code_new = torch.cat((h_code_new, c_code_channel), 1)
